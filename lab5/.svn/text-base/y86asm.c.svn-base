...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    if (IS_END(cur))
        return PARSE_ERR;
    cur2=cur;
    while(!IS_BLANK(cur2)&&!IS_END(cur2)&&(*cur2)!=',')
    {
       cur2++;
    }
...
int relocate(void)
{
	...
	case I_IRMOVL:
	    for(;i<4;i++)
	    {
		rtmp->y86bin->codes[2+i]=(sym->addr>>(8*i))&0xff;
	    }
	    i=0;
	    break;
	case I_JMP:case I_CALL:
	    for(;i<4;i++)
	    {
		rtmp->y86bin->codes[1+i]=(sym->addr>>(8*i))&0xff;
	    }
	    i=0;
	    break;
	case I_DIRECTIVE:
	switch(LOW(rtmp->y86bin->codes[0]))
	{
	    case D_DATA:
		for(;i<4;i++)
	        {
...
int binfile(FILE *out)
{
    ...
    int addr1=0,addr2=0;
    byte_t itype1=I_HALT,itype2=I_HALT;
    /* binary write y86 code to output file (NOTE: see fwrite()) */
    while(tmp)
    {
	if(tmp->y86bin.bytes!=0)
	{
	itype1=tmp->y86bin.codes[0];
	addr1=tmp->y86bin.addr;
	if((itype1!=HPACK(I_DIRECTIVE,D_POS)&&itype1!=HPACK(I_DIRECTIVE,D_ALIGN))||(itype2!=HPACK(I_DIRECTIVE,D_POS)&&itype2!=HPACK(I_DIRECTIVE,D_ALIGN)))
	{
    //printf("%x: ",addr1);
    //printf("%x",addr1-addr2);
	    while(addr1-addr2>0)
	    {
	        char a='\0';
	        fwrite(&a,1,1,out);
	...
	if(itype1==HPACK(I_DIRECTIVE,D_DATA))
	{
	    int i=0;
	    while(i<4)
	    {
		tmp->y86bin.codes[i]=tmp->y86bin.codes[i+1];
		i++;
	    }
	...
	addr2=tmp->y86bin.addr+tmp->y86bin.bytes;
	itype2=itype1;
	}
	tmp=tmp->next;
