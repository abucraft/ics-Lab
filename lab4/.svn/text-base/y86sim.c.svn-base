...
zero=(val==0);
sign=(val<0);
ovf=((argB<0==argA>0)&&(argB<0!=val<0));
break;
case A_AND:
zero=(val==0);
sign=(val<0);
...
regid_t regid_A,regid_B;
long_t imm;
long_t midval;
long_t oldesp;
long_t address;
if((icode<7&&icode>1)||icode==I_PUSHL||icode==I_POPL)
{
	if (!get_byte_val(sim->m, next_pc, &codefun)) {
        err_print("PC = 0x%x, Invalid instruction address", sim->pc);
        return STAT_ADR;
	}
regid_A=GET_REGA(codefun);
regid_B=GET_REGB(codefun);
next_pc++;
}
    /* get immediate if needed (4 bytes) */
if((icode<6&&icode>2)||icode==I_JMP||icode==I_CALL)
{
...
next_pc+=4;
	...
	set_reg_val(sim->r,regid_B,get_reg_val(sim->r,regid_A));
	sim->pc=next_pc;
	break;
      case I_IRMOVL: /* 3:0 F:regB imm */
	set_reg_val(sim->r,regid_B,imm);
	sim->pc=next_pc;
	break;
      case I_RMMOVL: /* 4:0 regA:regB imm */
	midval=get_reg_val(sim->r,regid_B);
	midval+=imm;
	if(!set_long_val(sim->m,midval,get_reg_val(sim->r,regid_A)))
	{
	err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc,midval);
	return STAT_ADR;
	}
	sim->pc=next_pc;
	break;
      case I_MRMOVL: /* 5:0 regB:regA imm */
	address=get_reg_val(sim->r,regid_B);
	address+=imm;
	if(!get_long_val(sim->m,address,&midval))
	{
	err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc,address);
	return STAT_ADR;
	}
	set_reg_val(sim->r,regid_A,midval);
	sim->pc=next_pc;
	break;
      case I_ALU: /* 6:x regA:regB */
	midval=compute_alu(ifun,get_reg_val(sim->r,regid_A),get_reg_val(sim->r,regid_B));
	sim->cc=compute_cc(ifun,get_reg_val(sim->r,regid_A),get_reg_val(sim->r,regid_B),midval);
	set_reg_val(sim->r,regid_B,midval);
	sim->pc=next_pc;
	break;	
      case I_JMP: /* 7:x imm */
	if(cond_doit(sim->cc,ifun))
	...
	break;
      case I_CALL: /* 8:x imm */
	oldesp=get_reg_val(sim->r,REG_ESP);
	oldesp-=4;
	set_reg_val(sim->r,REG_ESP,oldesp);
	if(!set_long_val(sim->m,oldesp,next_pc))
	{
	err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc,oldesp)
	return STAT_ADR;
	}
	sim->pc=imm;
	break;
      case I_RET: /* 9:0 */
	oldesp=get_reg_val(sim->r,REG_ESP);
	get_long_val(sim->m,oldesp,&midval);
	oldesp+=4;
	set_reg_val(sim->r,REG_ESP,oldesp);
	sim->pc=midval;
	break;
      case I_PUSHL: /* A:0 regA:F */
	midval=get_reg_val(sim->r,regid_A);
	oldesp=get_reg_val(sim->r,REG_ESP);
	oldesp-=4;
	set_reg_val(sim->r,REG_ESP,oldesp);
	if(!set_long_val(sim->m,oldesp,midval))
	{
	err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc,oldesp);
	return STAT_ADR;
	}
	...
	oldesp=get_reg_val(sim->r,REG_ESP);
	if(!get_long_val(sim->m,oldesp,&midval))
	{
	err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc,oldesp);
	return STAT_ADR;
	}
