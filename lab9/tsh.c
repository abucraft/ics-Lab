...
void do_bgfg(char **argv) 
{
	...
	{
		id=atoi(&argv[1][1]);
		if((tempjob=getjobjid(jobs,id))==NULL)
		{
	...
	{
		id=atoi(argv[1]);
		if((tempjob=getjobpid(jobs,id))==NULL)
		{
...
{
	struct job_t* tempjob;
	while((tempjob=getjobpid(jobs,pid))!=NULL){
		if(tempjob->state!=FG)
			break;
		sleep(1);
	}
...
void sigint_handler(int sig) 
{
	...
	pid=fgpid(jobs);
	kill(-pid,SIGINT);
	tempjob=getjobpid(jobs,pid);
	printf("Job [%d] (%d) terminated by signal %d\n",tempjob->jid,tempjob->pid,SIGINT);
	deletejob(jobs,pid);
    return;
}
